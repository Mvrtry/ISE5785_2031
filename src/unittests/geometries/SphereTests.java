...
class SphereTests {
    ...
    public void testFindIntersections() {
        ...
        assertEquals(1,
                sphere.findIntersections(new Ray(new Point(0.5, 0.5, 0), new Vector(3, 1, 0))).size(),
        ...
        var result = sphere.findIntersections(new Ray(new Point(1, -2, 0), new Vector(0, 1, 0)));
        assertNotNull(result, "Line through center, ray crosses sphere");
        assertEquals(2, result.size(), "Line through center, ray crosses sphere: wrong number of points");

        // TC22: Ray starts at sphere and goes inside (1 points)
        assertEquals(1,
                sphere.findIntersections(new Ray(new Point(1, -1, 0), new Vector(0, 1, 0))).size(),
                "Line through center, ray from sphere inside");

        // TC23: Ray starts inside (1 points)
        assertEquals(1,
                sphere.findIntersections(new Ray(new Point(1, 0.5, 0), new Vector(0, 1, 0))).size(),
                "Line through center, ray from inside");

        // TC24: Ray starts at the center (1 points)
        assertEquals(1,
                sphere.findIntersections(new Ray(new Point(1, 0, 0), new Vector(0, 1, 0))).size(),
                "Line through center, ray from center");
        ...
        assertEquals(1,
                sphere.findIntersections(new Ray(new Point(0.5, 0, 0), new Vector(0, 0, 1))).size(),
                "Ray starts inside, orthogonal to ray head -> sphere center line");
    }
}
